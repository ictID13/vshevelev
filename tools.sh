...
ALL_KEYWORDS=("${K_MATTER}" "${K_OPEN_FACT}" "${K_INSTALL}" "${K_RFAV}" "${K_GETPROP}" "${K_GET_IP}" "${K_MONKEY}" "${K_CONNECT}" "${K_DISCONNECT}" "${K_CONNECT2ALL_SEND_BROADCAST}" "${K_INSTALL_PAK}" "${K_LOGCAT}" "${K_UNINST}" "${K_UNINST_PAK}" "${K_UNINST_DIFF}" "${K_SEND}" "${K_STOP_APP}" "${K_GET_NAME}" "${K_GET_VERS}" "${K_DIFF_VERSION}" "${K_CLEAR_DATA}" "${K_FIND}" "${K_HELP}")
...
stop_app(){
  echo  -n "Остановка abox.store.client :"
  adb shell am force-stop ${PKG:=abox.store.client}
  adb shell kill $(adb shell pidof abox.store.client)
  echo "Done"
}
...
#Установка prop
setprop(){
  echo -n "Установка persist.sys.registered/persist.sys.licensed  :"
...
pull_file() {
  PULL=$(/Users/Viktor/Documents/PROJECTS/ATLAS_launcher/\#atlas-02.04.2021/Atlas.Launcher-standard-launcher-release.apk)
  echo -n "Pulling apk"
  adb pull  ${PULL} 2>/dev/null 1>/dev/null
  echo
  pwd
}

push_file() {
 PUSH=$(/Users/Viktor/Documents/PROJECTS/ATLAS_launcher/\#atlas-02.04.2021/Atlas.Launcher-standard-launcher-release.apk)
 EXDIR=$(/Users/Viktor/)
  echo -n "Pushing  apk :"
  adb push  ${PULL}  ${EXDIR} 2>/dev/null 1>/dev/null
  echo "Done"
}
...
send_broadcast_store_client(){
      echo -n "Broadcast for abox.store.client : "
      adb shell am broadcast -a abox.store.client.ACTION_START -n abox.store.client/.receiver.StoreBroadcastReceiver  2> /dev/null 1> /dev/null
      if [ $? -ne 0 ] ; then
        echo "Failed "
        else
        echo "Sended"
...
#Последовательный перебор всех  вариантов вызова factory_menu
open_factory(){
echo  -n "send HiKeen(RTK2851/RTK2842) SOURCE>2580 : "
  adb shell am start -n 'com.hikeen.factorymenu/com.hikeen.factorymenu.FactoryMenuActivity'  2> /dev/null 1> /dev/null
  check
  sleep 1
echo -n "send CVTE(MTK55xx/SK506/SK706) HOME.SOURCE>ATV>MENU>1147 : "
  ...
  check
  sleep 1
echo -n "send KTC(6681) SOURCE>ATV>MENU>8202 :"
